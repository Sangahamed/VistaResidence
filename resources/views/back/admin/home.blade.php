...
<head>
    ...
    <style>/* Masquer la sidebar par défaut en mode mobile */
#sidebar {
    position: fixed; /* Position fixe en mode mobile */
    left: -100%; /* Masquer la sidebar par défaut */
    height: 100vh; /* Hauteur totale de la vue */
    width: 16rem; /* Largeur de la sidebar (64 * 0.25rem) */
    transition: left 0.3s ease-in-out; /* Animation fluide */
    z-index: 50; /* Placer la sidebar au-dessus du contenu */
}

/* Afficher la sidebar en mode mobile */
#sidebar:not(.hidden) {
    left: 0; /* Déplacer la sidebar vers la droite */
}

/* Styles pour le mode desktop */
@media (min-width: 1024px) {
    #sidebar {
        position: static; /* Position statique en mode desktop */
        left: 0; /* Toujours visible */
    }

    #overlay {
        display: none !important; /* Masquer l'overlay en mode desktop */
    }

    /* Masquer le bouton en mode desktop */
    #mobileMenuBtn {
        display: none;
    }
}</style>
