...
class LogUserActivity
{
    ...
    protected function getDeviceInfo(Request $request): array
    {
        ...
        return [
            ...
            'os' => $this->getOSName($agent),
        ];
    }

    protected function getBrowserName($agent): string
    {
        if (preg_match('/MSIE/i', $agent) || preg_match('/Trident/i', $agent)) return 'Internet Explorer';
        elseif (preg_match('/Firefox/i', $agent)) return 'Firefox';
        elseif (preg_match('/Chrome/i', $agent)) return 'Chrome';
        elseif (preg_match('/Safari/i', $agent)) return 'Safari';
        elseif (preg_match('/Opera/i', $agent)) return 'Opera';
        else return 'Unknown';
    }

    protected function getOSName($agent): string
    {
        if (preg_match('/windows|win32/i', $agent)) return 'Windows';
        elseif (preg_match('/macintosh|mac os x/i', $agent)) return 'Mac';
        elseif (preg_match('/linux/i', $agent)) return 'Linux';
        elseif (preg_match('/android/i', $agent)) return 'Android';
        elseif (preg_match('/iphone/i', $agent)) return 'iPhone';
        elseif (preg_match('/ipad/i', $agent)) return 'iPad';
        else return 'Unknown';
    }
}
